...
Semgrep matches the first occurrence of `bar` in the test code as this object falls under the scope of `foo` and `...`. The ellipsis operator does not match the second occurrence of `bar` or the first occurrence of `baz` as they are not inside of the function definition, and are therefore not inside the scope of the ellipsis operator.
...
However, you can also match a single item using ellipsis `...` operator.
